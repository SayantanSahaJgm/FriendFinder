...
export async function sendFriendRequest(targetUserId: string): Promise<FriendRequestResult> {
  try {
    ...
    try {
      // Prefer direct emit to the Socket.IO server's /emit bridge when available
      ...
      console.error('Direct emit failed, falling back to proxy /api/socket.io — error:', socketError)
      try {
        ...
        console.error('Failed to send real-time notification via proxy as well:', proxyErr)
      }
  ...
  try {
    ...
    if (action === 'accept') {
      ...
      try {
        ...
        console.error('Direct emit failed for friend_response, falling back to proxy — error:', socketError)
        try {
          await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3003'}/api/socket.io`, {
            method: 'POST',
            headers: {
          ...
          console.error('Failed to send friend_response via proxy as well:', proxyErr)
        }
      ...
      try {
        ...
        console.error('Direct emit failed for friend_response rejection, falling back to proxy — error:', socketError)
        try {
          await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3003'}/api/socket.io`, {
            method: 'POST',
            headers: {
          ...
          console.error('Failed to send friend_response rejection via proxy as well:', proxyErr)
        }
        // Continue without failing the response
      }
