...
export default function DiscoverPage() {
  ...
  const handleMethodToggle = (method: DiscoveryMethod) => {
    ...
    const newUrl = params.toString() ? `/dashboard/discover?${params.toString()}` : '/dashboard/discover';
  ...
  const handleBluetoothButtonClick = async () => {
    // Right panel: automatic Bluetooth scanning for nearby users with the app
    // (for web: uses DB-based discovery; for native mobile: uses real Bluetooth scanning)
    await findBluetoothUsers();
  };

  const handleEnterPairingCode = async () => {
    try {
      const code = window.prompt('Enter the 6-digit pairing code you received:')
      if (!code) return

      const resp = await bluetoothService.pairWithCode(code.trim())
      if (resp && resp.success) {
        toast.success(resp.message || 'Pairing code accepted — friend request sent')
        // Refresh lists
        await findBluetoothUsers()
      } else {
        toast.error(resp.message || 'Failed to pair with code')
      }
    } catch (error: any) {
  ...
  // Subscribe to real-time bluetooth updates via Socket.IO
  const { onNearbyBluetoothUpdate, onNearbyBluetoothCleared } = useSocket()

  useEffect(() => {
    // Only subscribe when Bluetooth discover tab is active
    if (activeMethod !== 'bluetooth') return

    const offUpdate = onNearbyBluetoothUpdate(async (payload: any) => {
      console.log('Received bluetooth_update socket event:', payload)
      // Refresh nearby users when an update arrives
      await findBluetoothUsers()
    })

    const offCleared = onNearbyBluetoothCleared(async (payload: any) => {
      console.log('Received bluetooth_cleared socket event:', payload)
    ...
    return () => {
      ...
      if (offCleared) offCleared()
    }
  }, [activeMethod, onNearbyBluetoothUpdate, onNearbyBluetoothCleared])
        ...
        <div>
          {activeMethod === "gps" ? (
            ...
            <Card>
              ...
              <CardContent>
                {/* Scan Nearby Friends - Automatic Bluetooth Discovery */}
                <div className="mb-4">
                  <Button
                    onClick={handleBluetoothButtonClick}
                  ...
                  <p className="text-xs text-gray-500 mt-2 text-center">
                    Finds nearby users with Bluetooth ON and the app installed
                  </p>
                </div>
                ...
                {isBluetoothLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-400">
                      Click "Scan Nearby Friends" to find people with Bluetooth ON and the app installed
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
