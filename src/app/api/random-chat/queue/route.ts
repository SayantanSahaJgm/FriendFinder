...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingSession = await RandomChatSession.findActiveSessionForUser(userId);
    if (existingSession) {
      return NextResponse.json(
    ...
    const recentReportsCount = await (RandomChatReport as any).hasRecentReports(userId, 24);
    ...
    const potentialMatch = await (RandomChatQueue as any).findNextMatch(userId, validatedPreferences);
    ...
    if (potentialMatch) {
      ...
      const queueStats = await (RandomChatQueue as any).getQueueStats();
      const currentTypeStats = queueStats.find((stat: any) => stat._id === chatType);
