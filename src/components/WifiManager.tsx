...
export default function WifiManager({ onWifiUpdated }: WifiManagerProps = {}) {
  ...
  const handleGenerateCode = async () => {
    if (!networkName.trim()) {
      toast.error("Please enter your WiFi network name");
      return;
    }
    ...
    try {
      ...
      console.log('📶 [WiFi UI] Received response:', result);
      console.log('📶 [WiFi UI] Code:', result.pairingCode, 'Expires:', result.expiresAt, 'Type:', typeof result.expiresAt);
  ...
  const handleSubmitPairingCode = async () => {
    ...
    try {
      setIsPairing(true);
      console.log('📶 [WiFi UI] Attempting to pair with code:', pairingCode);
      const result = await wifiService.pairWithCode(pairingCode.trim());
      
      console.log('📶 [WiFi UI] Pair result:', result);
      if (result.success) {
      ...
      console.error("🔴 [WiFi UI] Pairing error:", error);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {!generatedCode ? (
            <div className="space-y-3">
              ...
              <Button
                ...
                disabled={isGenerating || !networkName.trim()}
                className="w-full h-11"
              >
                {isGenerating ? (
                  <>
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Share this 6-digit code with someone nearby to connect via WiFi
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Enter the code from someone on the same WiFi network to send a friend request
          </p>
        </div>
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="text-xs text-blue-800">
              ...
              <p className="leading-relaxed">
                Pairing codes expire after 5 minutes and can only be used once for secure connections.
              </p>
            </div>
          </div>
        </div>
