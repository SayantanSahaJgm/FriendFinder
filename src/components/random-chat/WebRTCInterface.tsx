          ...
          <div className="text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-xs">
                ...
                {partnerVerificationConfidence != null && (
                  <span className="text-muted-foreground">{Math.round(partnerVerificationConfidence * 100)}%</span>
                )}
              </span>
            </div>
          </div>
              ...
              {localStream && (
                <div className="absolute top-4 right-4 w-32 h-24 sm:w-48 sm:h-36 bg-black rounded-lg overflow-hidden border-2 border-white/20">
                  <video
                    ...
                    className="w-full h-full object-cover"
                  />
                  {/* Partner verification badge (shows whether partner reported verified state) */}
                  <div className="absolute bottom-2 left-2 bg-white/90 rounded-full px-2 py-1 flex items-center gap-2 text-xs">
                    {partnerVerified == null ? (
                      <span className="text-foreground/70">Verification: —</span>
                    ) : partnerVerified ? (
                      <span className="text-green-600 animate-pulse">Verified</span>
                    ) : (
                      <span className="text-red-600 animate-pulse">Unverified</span>
                    )}
                    {partnerVerificationConfidence != null && (
                      <span className="text-muted-foreground">{Math.round(partnerVerificationConfidence * 100)}%</span>
                    )}
                    {/* Last verification timestamp (if present) */}
                    {typeof partnerVerificationConfidence === 'number' && (
                      <span className="ml-2 text-[10px] text-muted-foreground">Updated</span>
                    )}
                  </div>
                  {!isVideoEnabled && (
