...
export default function TopHeader() {
  ...
  return (
  <header className="w-full bg-slate-900 text-white ff-white ff-white">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href={session?.user ? "/dashboard" : "/"} className="flex items-center space-x-2 hover:opacity-90 transition">
            <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full flex items-center justify-center">
              <span className="text-white ff-white ff-white font-bold text-lg">FF</span>
            </div>
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            <Link href={session?.user ? "/dashboard/notifications" : "/login"} className="relative">
              ...
              <span className="absolute -top-1 -right-1 bg-red-500 text-white ff-white ff-white text-xs rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center text-[10px] sm:text-xs">2</span>
            </Link>
            ...
            <Link href={session?.user ? "/dashboard/profile" : "/login"}>
              <Avatar className="h-8 w-8 sm:h-10 sm:w-10">
                <AvatarFallback className="bg-blue-500 text-white ff-white ff-white text-sm">
