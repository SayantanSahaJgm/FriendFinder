...
/**
 * Verification event interface
 */
export interface IRandomChatVerification {
  eventId: string;
  userId?: mongoose.Types.ObjectId; // optional if anonymous
  anonymousId?: string;
  isVerified: boolean;
  confidence?: number | null;
  timestamp: Date;
  signedToken?: string | null; // optional server-signed token
}
...
export interface IRandomChatSession extends Document {
  ...
  verifications?: IRandomChatVerification[];
...
const RandomChatSessionSchema = new Schema<IRandomChatSession>({
  ...
  verifications: {
    type: [new Schema<IRandomChatVerification>({
