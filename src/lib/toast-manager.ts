...
class EnhancedToastManager {
   ...
   */
  show(message: string, config: ToastConfig = {}): string | number | null {
   ...
   */
  success(message: string, config: Omit<ToastConfig, 'category'> = {}): string | number | null {
   ...
   * Show an error toast
   */
  error(message: string, config: Omit<ToastConfig, 'category'> = {}): string | number | null {
   ...
   */
  warning(message: string, config: Omit<ToastConfig, 'category'> = {}): string | number | null {
    return this.show(message, { 
      ...config, 
   ...
   * Show an info toast
   */
  info(message: string, config: Omit<ToastConfig, 'category'> = {}): string | number | null {
   ...
   */
  loading(message: string, config: Omit<ToastConfig, 'category'> = {}): string | number | null {
    return this.show(message, { 
      ...config, 
   ...
   */
  private displayToast(message: string, config: ToastConfig): string | number {
    const {
  ...
  let toastId: string | number;
    ...
    this.activeToasts.set(String(toastId), toast as any);
