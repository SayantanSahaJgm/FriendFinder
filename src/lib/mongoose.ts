...
const cached = global.mongooseCache as CachedConnection
...
async function dbConnect(): Promise<typeof mongoose> {
  ...
  if (!MONGODB_URI) {
    // Defer the error until an actual connection attempt is made so build-time
    // static analysis (Next.js page data collection) doesn't fail when the
    // environment variable is only provided at runtime (e.g., Render).
